...
export const getMealsHistory = (): Record<string, DayMeals> => {
  try {
    const STORAGE_KEY = getMealsKey();
    const data = localStorage.getItem(STORAGE_KEY);
    console.log('📖 getMealsHistory: key=', STORAGE_KEY, 'hasData=', !!data);
...
export const saveMealsHistory = (history: Record<string, DayMeals>): void => {
  try {
    const STORAGE_KEY = getMealsKey();
    console.log('💾 saveMealsHistory: key=', STORAGE_KEY, 'dates=', Object.keys(history));
